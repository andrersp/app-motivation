...
class Mock {
    ...
    private val langPt = MotivationConstants.LANGUAGE.PORTIGUESE
    ...
    fun getPhrase(categoryId: Int, lang: String): Phrase {
        if (categoryId == all) {
            val filtered = listPhrases.filter { it.lang == lang }
            return filtered[Random.nextInt(filtered.size)]
        }

        val filtered = listPhrases.filter { it.categoryId == categoryId }
        return filtered[Random.nextInt(filtered.size)]
